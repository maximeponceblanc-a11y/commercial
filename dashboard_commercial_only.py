...
# ========================================================
# CLASSIFICATION ABC (commune aux deux BU)
# ========================================================
...
df_pb = load_ponceblanc_data()
df_pb = calculer_classification_abc_par_annee(df_pb)

df_lbfi = load_lbfi_data()
df_lbfi = calculer_classification_abc_par_annee(df_lbfi)
...
tab_pb, tab_lbfi = st.tabs(["🏢 PONCEBLANC", "🏢 LBFI"])


# ========================================================
# FONCTIONS COMMUNES DE RENDU
# ========================================================
...
def extraire_kpis_annee(dataframe, annee_cible):
    ...
    return {"ca_devis": ca_devis, "ca_signe": ca_signe, "tx_ca": tx_succes_ca,
            "vol_devis": vol_devis, "vol_signe": vol_signe, "tx_vol": tx_succes_vol}


def render_bu(df, bu_key, has_commercial=False, has_matiere=False):
    """Rendu complet du dashboard pour une BU donnée."""

    # ---- BARRE LATÉRALE (dans le contexte de l'onglet actif) ----
    st.sidebar.header(f"🎛️ Filtres — {bu_key}")
    ...
    mask_temporel = (
        (df['Année Devis'].isin(annees_selectionnees)) &
        (df['Mois_Num'] >= mois_debut_num) &
        (df['Mois_Num'] <= mois_fin_num)
    )

    st.sidebar.markdown("### 👤 Commercial" if has_commercial else "### 📁 Dossier")

    if has_commercial and "Commercial" in df.columns:
        options_comm = sorted([str(x) for x in df["Commercial"].dropna().unique()])
        selected_commercial = st.sidebar.multiselect("Sélection du commercial", options=options_comm, key=f"comm_{bu_key}")
    ...
    search_client = st.sidebar.text_input("Rechercher un Nom Client", key=f"client_{bu_key}")
    search_devis = st.sidebar.text_input("Rechercher un N° de Devis", key=f"devis_{bu_key}")
    ...
    for col_df, label in [("Type de produit", "Type de produit"), ("Matière", "Matière")]:
        if col_df in df.columns and (col_df != "Matière" or has_matiere):
            options = sorted([str(x) for x in df[col_df].dropna().unique()])
            selected = st.sidebar.multiselect(label, options=options, key=f"{col_df}_{bu_key}")
            if selected:
    ...
    if vue_annuelle and len(annees_selectionnees) > 0:
        ...
        with col_g:
            ...
            if not df_kpis_all.empty:
                ...
                fig_yr.add_trace(go.Scatter(x=df_kpis_all['Année'], y=df_kpis_all['tx_ca'] / 100,
                    name="Taux succès (€)", mode='lines+markers+text',
                ...
                st.plotly_chart(fig_yr, use_container_width=True, key=f"fig_yr_{bu_key}")
            ...
            if not df_kpis_all.empty:
                ...
                fig_vol_yr.add_trace(go.Scatter(x=df_kpis_all['Année'], y=df_kpis_all['tx_vol'] / 100,
                    name="Taux succès (vol)", mode='lines+markers+text',
                ...
                fig_vol_yr.update_layout(barmode='group', xaxis=dict(type='category'), yaxis=dict(title="Nombre de devis"),
                    yaxis2=dict(title="Taux succès", tickformat=".0%", range=[0, 1.2], showgrid=False),
                    legend=dict(orientation="h", y=1.1), hovermode="x unified", height=380)
                st.plotly_chart(fig_vol_yr, use_container_width=True, key=f"fig_vol_yr_{bu_key}")
        ...
        with col_d:
            ...
            if not df_pie_all.empty:
                ...
                fig_pie_all = go.Figure(data=[go.Pie(labels=["Aucune commande"], values=[1], hole=.4,
            ...
            st.plotly_chart(fig_pie_all, use_container_width=True, key=f"fig_pie_all_{bu_key}")
            ...
            with tabs_val[idx]:
                ...
                if not df_annee.empty:
                    ...
                    with col_gauche:
                        ...
                        df_graph['Taux succès (€)'] = df_graph.apply(
                            lambda row: (row['CA commandes'] / row["Chiffre d'affaire"]) if row["Chiffre d'affaire"] > 0 else 0, axis=1)
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"fig_ca_{bu_key}_{annee_selectionnee}")
                        ...
                        df_graph_vol['Taux succès (vol)'] = df_graph_vol.apply(
                            lambda row: (row['Nb commandes'] / row['Nb devis']) if row['Nb devis'] > 0 else 0, axis=1)
                        ...
                        fig_vol.update_layout(barmode='group', xaxis=dict(type='category'), yaxis=dict(title="Nombre de devis", showgrid=True),
                            ...
                            legend=dict(orientation="h", y=1.1, x=0), hovermode="x unified", height=380)
                        st.plotly_chart(fig_vol, use_container_width=True, key=f"fig_vol_{bu_key}_{annee_selectionnee}")
                    ...
                    with col_droite:
                        ...
                        st.plotly_chart(fig_pie, use_container_width=True, key=f"fig_pie_{bu_key}_{annee_selectionnee}")
                        ...
                        df_table_fmt["CA"] = df_table_fmt["CA"].map(lambda x: f"{x:,.0f} €".replace(',', ' '))
                        st.dataframe(df_table_fmt, use_container_width=True, hide_index=True, height=440)
                else:
    ...
    nb_cols = 3 if has_matiere else 2
    cols_pie = st.columns(nb_cols)
    ...
    with cols_pie[0]:
        if "Nom Client" in df_signe.columns and not df_signe.empty:
            ...
            couleurs = px.colors.qualitative.Set2[:min(6, n)] + ["#e2e8f0"] * max(0, n - 6)
            fig_c1 = go.Figure(data=[go.Pie(
                labels=legend_labels, values=df_c["Valeur"], hole=0.4, textinfo="percent",
            ...
            fig_c1.update_layout(
                title=dict(text="CA commandes par Client <span style='font-size:11px;color:#94a3b8'>(top 6 nommés)</span>", font=dict(size=13), x=0),
                legend=dict(orientation="h", y=-0.15, x=0), margin=dict(t=40, b=10, l=10, r=10), height=340)
            st.plotly_chart(fig_c1, use_container_width=True, key=f"fig_c1_{bu_key}")
        else:
    ...
    with cols_pie[1]:
        if "Type de produit" in df_signe.columns and not df_signe.empty:
            ...
            fig_c2 = go.Figure(data=[go.Pie(
                labels=df_p2["Label"], values=df_p2["Valeur"], hole=0.4, textinfo="percent",
            ...
            fig_c2.update_layout(
                ...
                legend=dict(orientation="h", y=-0.15, x=0), margin=dict(t=40, b=10, l=10, r=10), height=340)
            st.plotly_chart(fig_c2, use_container_width=True, key=f"fig_c2_{bu_key}")
        else:
            st.info("Colonne 'Type de produit' non disponible.")

    if has_matiere and nb_cols == 3:
        with cols_pie[2]:
            if "Matière" in df_signe.columns and not df_signe.empty:
                ...
                fig_c3 = go.Figure(data=[go.Pie(
                    labels=df_p3["Label"], values=df_p3["Valeur"], hole=0.4, textinfo="percent",
                    hovertemplate="<b>%{label}</b><br>CA : %{value:,.0f} €<br>Part : %{percent}<extra></extra>")])
                fig_c3.update_layout(
                    title=dict(text="CA commandes par Matière", font=dict(size=13), x=0),
                    legend=dict(orientation="h", y=-0.15, x=0), margin=dict(t=40, b=10, l=10, r=10), height=340)
                st.plotly_chart(fig_c3, use_container_width=True, key=f"fig_c3_{bu_key}")
            else:
    ...
    if 'Délai devis ouverture' in df_filtered_base.columns:
        ...
        if not df_delai.empty:
            if vue_annuelle:
                df_delai_groupe = df_delai.groupby('Année Devis')['Délai devis ouverture'].mean().reset_index()
                df_delai_groupe.columns = ['Période', 'Délai moyen']
                df_delai_groupe['Période'] = df_delai_groupe['Période'].astype(str)
                x_label = "Année"
            else:
                df_delai_groupe = df_delai.groupby('Mois_Nom')['Délai devis ouverture'].mean().reindex(mois_selectionnes_bornes, fill_value=None).reset_index()
                df_delai_groupe.columns = ['Période', 'Délai moyen']
                x_label = "Mois"
            ...
            with col_delai_graph:
                ...
                fig_delai.add_trace(go.Bar(
                    x=df_delai_groupe['Période'], y=df_delai_groupe['Délai moyen'].round(1),
                    marker_color='#6366f1',
                    text=df_delai_groupe['Délai moyen'].round(1).apply(lambda v: f"{v:.1f} j".replace('.', ',') if pd.notna(v) else ""),
                    textposition='outside',
                    hovertemplate="<b>%{x}</b><br>Délai moyen : %{y:.1f} jours<extra></extra>"))
                fig_delai.update_layout(xaxis=dict(title=x_label, type='category'),
                    yaxis=dict(title="Jours", showgrid=True), hovermode="x unified", height=360, margin=dict(t=20, b=40))
                st.plotly_chart(fig_delai, use_container_width=True, key=f"fig_delai_{bu_key}")
            ...
            with col_delai_table:
                ...
                df_detail_delai = df_delai[cols_present].copy().rename(columns={
                    "DEVIS N°": "N° Devis", "Année Devis": "Année", "Mois_Nom": "Mois", "Délai devis ouverture": "Délai (j)"})
    ...
    def construire_resume_filtres():
        parties = [f"BU : {bu_key}", f"Maille : {'Annuelle' if vue_annuelle else 'Mensuelle'}"]
        if annees_selectionnees:
            parties.append(f"Années : {', '.join(str(a) for a in sorted(annees_selectionnees))}")
        if not vue_annuelle:
            parties.append(f"Mois : {start_month} → {end_month}" if start_month != end_month else f"Mois : {start_month}")
        if has_commercial and selected_commercial:
        ...
        parties.append(f"Édité le {datetime.datetime.now().strftime('%d/%m/%Y à %H:%M')}")
        return "  ·  ".join(parties)

    st.markdown("---")
    st.markdown(
        f"<p style='font-size:0.75em; color:#94a3b8; text-align:center; margin-top:4px;'>"
        f"🖨️ {construire_resume_filtres()}</p>",
        unsafe_allow_html=True
    )
...
with tab_pb:
    render_bu(df_pb, bu_key="PONCEBLANC", has_commercial=True, has_matiere=True)

with tab_lbfi:
    render_bu(df_lbfi, bu_key="LBFI", has_commercial=False, has_matiere=False)