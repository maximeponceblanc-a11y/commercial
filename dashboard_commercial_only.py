...
if vue_annuelle and len(annees_selectionnees) > 0:
    ...
    if kpis_courant:
        ...
        with c1:
            st.metric(
                label=f"CA Signé {annee_courante}",
    ...
    with col_g:
        # Graphique CA Devisé / Signé par année + taux succès
        ...
        if not df_kpis_all.empty:
            ...
            fig_yr.add_trace(go.Bar(x=df_kpis_all['Année'], y=df_kpis_all['ca_signe'], name="CA Signé", marker_color='#4ed2e6'), secondary_y=False)
        ...
        if not df_kpis_all.empty:
            ...
            fig_vol_yr.add_trace(go.Bar(x=df_kpis_all['Année'], y=df_kpis_all['vol_signe'], name="Nb devis signés", marker_color='#ffcc66'), secondary_y=False)
    ...
    with col_d:
        ...
        if not df_kpis_all.empty:
            ...
            df_recap['CA Signé'] = df_recap['ca_signe'].map(lambda x: f"{x:,.0f} €".replace(',', ' '))
            ...
            df_recap['Devis signés'] = df_recap['vol_signe'].astype(int)
            df_recap['Devis émis'] = df_recap['vol_devis'].astype(int)
            df_recap['Tx Succès (vol)'] = df_recap['tx_vol'].map(lambda x: f"{x:.2f} %".replace('.', ','))
            st.dataframe(df_recap[['Année', 'CA Signé', 'CA Devisé', 'Tx Succès (€)', 'Devis signés', 'Devis émis', 'Tx Succès (vol)']], use_container_width=True, hide_index=True)
...
# ========================================================
st.divider()
st.subheader("Répartition du CA Signé")
...
# --- Camembert 1 : CA signé par client (top 6 en couleur dans légende) ---
with col_p1:
    if "Nom Client" in df_signe.columns and not df_signe.empty:
        ...
        fig_c1.update_layout(
            title=dict(text="CA signé par Client <span style='font-size:11px;color:#94a3b8'>(top 6 nommés, survol pour détail)</span>", font=dict(size=13), x=0),
...
# --- Camembert 2 : CA signé par type de produit ---
with col_p2:
    if "Type de produit" in df_signe.columns and not df_signe.empty:
        ...
        fig_c2.update_layout(
            title=dict(text="CA signé par Type de produit", font=dict(size=13), x=0),
            legend=dict(orientation="h", y=-0.15, x=0),
            margin=dict(t=40, b=10, l=10, r=10),
            height=340,
        )
...
# --- Camembert 3 : CA signé par matière ---
with col_p3:
    if "Matière" in df_signe.columns and not df_signe.empty:
        ...
        fig_c3.update_layout(
            title=dict(text="CA signé par Matière", font=dict(size=13), x=0),
            legend=dict(orientation="h", y=-0.15, x=0),
            margin=dict(t=40, b=10, l=10, r=10),
            height=340,
        )
